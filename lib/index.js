...
function checkPhp(){
  ...
  const child = execFile(php,["-v"], (err, stdout, stderr) => {
    if (err){
      log("The " + php + " command failed!","fail");
      throw new Error(err);
    }
    checkFiles();
  });
...
function progress(){
  clear();
  if(progressCount == 7) progressCount = 0
  if(progressCount == 0) log("Waiting","busy")
  if(progressCount == 1) log("Waiting.","busy")
  if(progressCount == 2) log("Waiting..","busy")
  if(progressCount == 3) log("Waiting...","busy")
  if(progressCount == 4) log("Waiting....","busy")
  if(progressCount == 5) log("Waiting.....","busy")
  if(progressCount == 6) log("Waiting......","busy")
  progressCount++
}
