...
function checkFiles() {
  // Get all entity files and group them
  var walker

  walker = walk.walk(path.join(exports.workingDir, 'src'))

  walker.on('directories', function(root, dirStats, next) {
    var dirArray = root.split(path.sep)
    if (dirArray.pop() == 'Entity') {
      var sep = root.split(path.sep)
      var finalcmdpath = ''
      var hitsrc = false
  ...
  inquirer.prompt([
    {
      ...
      choices: ['Generate Entities' , 'Create Database', 'Load Fixtures', 'Back'],
      when: function(answers) {
        return answers.comments !==  'Nope, all good!'
      },
    },
    ...
    if (answers.command == 'Generate Entities')
      entitiesPrompt()
...
// Prompt shown user selects 'Generate Entities' in doctrine submenu
function entitiesPrompt() {
  var choices = []
  for (i = 0; i < entities.length; i ++) {
    choices.push(entities[i])
  }
  ...
  inquirer.prompt([
    {
      type: 'list',
      name: 'command',
      message: 'What entities would you like to generate?',
      choices: choices,
      when: function(answers) {
        return answers.comments !==  'Nope, all good!'
      },
    ...
    command('doctrine:generate:entities ' + answers.command, 'Entities generated for ' + answers.command, true)
  })
}
