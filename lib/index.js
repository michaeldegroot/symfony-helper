...
function doctrinePrompt(){
  inquirer.prompt([
    ...
    if(answers.command == "Load Fixtures") command("doctrine:fixtures:load -n","All fixtures were successfully installed.",true)
    if(answers.command == "Create Database") command("doctrine:database:create -n","Database successfully created.",true)
  })
}
...
function installAssetsPrompt(){
  inquirer.prompt([
    ...
    if(answers.command == "Back") reset()
    if(answers.command == "Install") command("assets:install","All assets were successfully installed.",true)
    if(answers.command == "Dump") command("assetic:dump","All assets were successfully dumped.",true)
  })
}
...
function clearCachePrompt() {
  inquirer.prompt([
    ...
    if(answers.command == "Back") reset()
    if(answers.command == "All Cache") clearCache("all")
    if(answers.command == "Dev Cache") clearCache("dev")
    if(answers.command == "Prod Cache") clearCache("prod")
  })
}
...
function clearCache(env){
    ...
    if(env == "all") var clear = path.join(exports.workingDir,"app","cache")
    if(env == "prod") var clear = path.join(exports.workingDir,"app","cache","prod")
    if(env == "dev") var clear = path.join(exports.workingDir,"app","cache","dev")
...
function progress(){
  ...
  this.start = function(){
    this.interval = setInterval(function(that){
      ...
      clear()
      if(that.progressCount >= that.progressArray.length) that.progressCount = 0
...
function clear() {
  if(!process.stdout.getWindowSize) return
	var i
  var lines
	var stdout = ""
	if (windows === false){
